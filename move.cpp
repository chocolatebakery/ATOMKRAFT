...
const string move_to_string(Move m) {
	  ...
	  if (move_is_short_castle(m))
	      return from == SQ_E1 ? "e1g1-scw" : "e8g8-scb";

	  if (move_is_long_castle(m))
	      return from == SQ_E1 ? "e1c1-lcw" : "e8c8-lcb";
	  ...
	  return square_to_string(from) + square_to_string(to) + promotion + ep;
}
...
#ifndef NDEBUG
  int failstep;
  int pos_is_ok = pos.is_ok(&failstep);
  if (!pos_is_ok) cout << "failstep: " << failstep << endl;
#endif
  ENDNEW
  assert(pos_is_ok);
  assert(move_is_ok(m));
      ...
      if (pt != PAWN)
      {
          ...
          for (MoveStack* cur = mlist; cur != last; cur++)
                  ...
                  && pos.type_of_piece_on(move_from(cur->move)) == pt)
              {
                  ...
                  STARTNEW
                  // something like knights on f7 and b5 moving to d6 is not regarded in
                  // the original stockfish code
                  if (   square_file(move_from(cur->move)) != square_file(from)
                	  && square_rank(move_from(cur->move)) != square_rank(from)) {
                	  r++;
                  }
                  ENDNEW
              }
