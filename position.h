...
struct StateInfo {
  ...
  Key key;
  Bitboard checkersBB;
  NEW ExplosionData expl; 	// stores all explodes pieces
  StateInfo* previous;
};
...
private:
  ...
  void do_capture_move(Key& key, PieceType capture, Color them, Square to, Square from, bool ep);
  NEW void remove_piece(Key& key, Color color, Square square, PieceType type);
  NEW void restore_piece(Color color, Square square, PieceType type);
  ...
  //ExplosionSquares miau;
  
  // Static variables
  static ExplosionData captureList[MaxGameLength];
