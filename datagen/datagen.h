...
namespace config {
    ...
    constexpr int VERIFICATION_DEPTH = 1;             // Quick verification
    constexpr int64_t VERIFICATION_NODES = 2500;     // 50k nodes max for verification
    constexpr int DATAGEN_DEPTH = 1;                  // Main search depth
    constexpr int64_t DATAGEN_NODES = 2500;          // 10k nodes per move
    constexpr int DATAGEN_TIME_MS = 50;              // 100ms per move
    ...
    constexpr int MAX_RANDOM_PLIES = 5;

    // Reporting
    constexpr int GAMES_PER_REPORT = 100;              // Report progress every N games
}
